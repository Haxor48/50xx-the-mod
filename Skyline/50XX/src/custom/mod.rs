...
pub unsafe fn jabCancels(boma: &mut smash::app::BattleObjectModuleAccessor, status_kind: i32, motion_kind: u64, fighter_kind: i32, cat1: i32, stick_value_x: f32, stick_value_y: f32) { //Jab Cancels
    ...
    if fighter_kind == *FIGHTER_KIND_CLOUD {
        if status_kind == *FIGHTER_STATUS_KIND_SPECIAL_S {
            if AttackModule::is_infliction_status(boma, *COLLISION_KIND_MASK_HIT) {
                if (stick_value_x * PostureModule::lr(boma) < 0.5 || ControlModule::check_button_off(boma, *CONTROL_PAD_BUTTON_SPECIAL)) && (cat1 & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_S) == 0 {
                    CancelModule::enable_cancel(boma);
                }
            }
        }
...
pub unsafe fn ivyHeals(boma: &mut smash::app::BattleObjectModuleAccessor, status_kind: i32, fighter_kind: i32, motion_kind: u64) { //Ivysaur healing and meter gain
    if fighter_kind == *FIGHTER_KIND_PFUSHIGISOU {
        ...
        if ![*FIGHTER_KIND_PLIZARDON, *FIGHTER_KIND_PZENIGAME].contains(&fighter_kind) || DamageModule::damage(boma, 0) != 0.0 {
            AMOUNTSOLAR[get_player_number(boma)] = 0;
        }
    }
}
...
pub unsafe fn squirtleSideB (boma: &mut smash::app::BattleObjectModuleAccessor, status_kind: i32, situation_kind: i32, fighter_kind: i32, motion_kind: u64, cat1: i32) {
    if fighter_kind == *FIGHTER_KIND_PZENIGAME {
        ...
        if status_kind == *FIGHTER_PZENIGAME_STATUS_KIND_SPECIAL_S_HIT {
            ...
            HITSIDEB[get_player_number(boma)] = true;
            StatusModule::change_status_request_from_script(boma, *FIGHTER_STATUS_KIND_JUMP_SQUAT, true);
        }
        else if status_kind != *FIGHTER_STATUS_KIND_JUMP_SQUAT {
            HITSIDEB[get_player_number(boma)] = false;
        }
...
pub unsafe fn moonwalking (boma: &mut smash::app::BattleObjectModuleAccessor, status_kind: i32, stick_value_x: f32) {
    if status_kind == *FIGHTER_STATUS_KIND_DASH {
        if stick_value_x * PostureModule::lr(boma) < -0.3 {
            ...
            let reverse_speed = Vector3f{x: 0.55 * stick_value_x * PostureModule::lr(boma), y: 0.0, z: 0.0};
...
pub unsafe fn animPortFix (lua_state: u64, l2c_agent: &mut L2CAgent, boma: &mut smash::app::BattleObjectModuleAccessor, situation_kind: i32, fighter_kind: i32, motion_kind: u64, stick_value_y: f32) {
    if fighter_kind == *FIGHTER_KIND_MARIO {
        ...
        if [hash40("speciallw"), hash40("speciallwr"), hash40("specialairlw"), hash40("specialairlwr")].contains(&motion_kind) {
            if MotionModule::frame(boma) > 1.0 {
                if ControlModule::check_button_trigger(boma, *CONTROL_PAD_BUTTON_SPECIAL) {
                    if situation_kind == *SITUATION_KIND_GROUND {
                        if PostureModule::lr(boma) < 0.0 {
                            MotionModule::change_motion(boma, Hash40{hash: hash40("laddercatchl")}, 0.0, 1.0, false, 0.0, false, false);
                        }
                        else {
                            MotionModule::change_motion(boma, Hash40{hash: hash40("laddercatchr")}, 0.0, 1.0, false, 0.0, false, false);
                        }
                    }
                    else {
                        if PostureModule::lr(boma) < 0.0 {
                            MotionModule::change_motion(boma, Hash40{hash: hash40("laddercatchairl")}, 0.0, 1.0, false, 0.0, false, false);
                        }
                        else {
                            MotionModule::change_motion(boma, Hash40{hash: hash40("laddercatchairr")}, 0.0, 1.0, false, 0.0, false, false);
                        }
                    }
                }
            }
...
pub unsafe fn superJumps (boma: &mut smash::app::BattleObjectModuleAccessor, status_kind: i32, fighter_kind: i32, cat1: i32) {
    if [*FIGHTER_STATUS_KIND_ATTACK_HI3, *FIGHTER_STATUS_KIND_ATTACK_HI3, *FIGHTER_STATUS_KIND_ATTACK_100].contains(&status_kind) {
        if fighter_kind == *FIGHTER_KIND_KEN {
            if AttackModule::is_infliction_status(boma, *COLLISION_KIND_MASK_HIT) {
...
pub unsafe fn sephirothEdits (boma: &mut smash::app::BattleObjectModuleAccessor, fighter_kind: i32, motion_kind: u64) {
    if fighter_kind == *FIGHTER_KIND_EDGE {
        ...
        if motion_kind == hash40("attack_air_lw") {
            ...
            if ZSSDAIR[get_player_number(boma)] {
                ...
                if WorkModule::is_flag(boma, *FIGHTER_STATUS_WORK_ID_FLAG_RESERVE_DIVE) {
                    ...
                    let speed_vector = smash::phx::Vector3f { x: 0.0, y: -1.0 * WorkModule::get_param_float(boma, 0, hash40("air_speed_y_stable")), z: 0.0 };
                    KineticModule::add_speed(boma, &speed_vector);
                }
            }
        }
    ...
    if fighter_kind == *FIGHTER_KIND_SNAKE {
        if ArticleModule::is_exist(boma, *FIGHTER_SNAKE_GENERATE_ARTICLE_C4) {
            ...
            if ArticleModule::is_exist(boma, *FIGHTER_SNAKE_GENERATE_ARTICLE_GRENADE) {
                ...
                if abs(c4pos.x - temp.x) <= 7.0 {
                    if !WorkModule::is_flag(boma, *FIGHTER_SNAKE_STATUS_SPECIAL_LW_EXPLODING_FLAG_C4_STARTUP) {
...
pub fn global_fighter_frame(fighter : &mut L2CFighterCommon) {
    unsafe {
        ...
        if ArticleModule::is_exist(boma, *FIGHTER_SNAKE_GENERATE_ARTICLE_C4) {
            let c4pos = ArticleModule::get_joint_pos(boma, *FIGHTER_SNAKE_GENERATE_ARTICLE_C4, Hash40{hash: hash40("trans")}, smash::cpp::root::app::ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL));
            println!("c4 x={}, y={}, z={}", c4pos.x, c4pos.y, c4pos.z);
        }
        if ArticleModule::is_exist(boma, *FIGHTER_SNAKE_GENERATE_ARTICLE_GRENADE) {
            let temp = ArticleModule::get_joint_pos(boma, *FIGHTER_SNAKE_GENERATE_ARTICLE_GRENADE, Hash40{hash: hash40("trans")}, smash::cpp::root::app::ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL));
            println!("grenade x={}, y={}, z={}", temp.x, temp.y, temp.z);
        }

        if ControlModule::check_button_on(boma, *CONTROL_PAD_BUTTON_GUARD) {
            println!("Wall touch line: {}, Is touch: {}", GroundModule::is_wall_touch_line(boma, get_player_number(boma) as u32), GroundModule::is_touch(boma, get_player_number(boma) as u32));
        }
        ...
        animPortFix(lua_state, &mut l2c_agent, boma, situation_kind, fighter_kind, motion_kind, stick_value_y);
