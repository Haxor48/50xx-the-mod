...
#[acmd_func(
    battle_object_category = BATTLE_OBJECT_CATEGORY_FIGHTER, 
    battle_object_kind = FIGHTER_KIND_SNAKE, 
    animation = "special_lw_blast",
    animcmd = "game_speciallwblast")]
pub fn snake_c4_blast_grnd(fighter: &mut L2CFighterCommon) {
    acmd!({
        frame(Frame=2)
        if(is_excute){
...
#[acmd_func(
    battle_object_category = BATTLE_OBJECT_CATEGORY_FIGHTER, 
    battle_object_kind = FIGHTER_KIND_SNAKE, 
    animation = "special_lw_squat_blast",
    animcmd = "game_speciallwsquatblast")]
pub fn snake_c4_blast_squat(fighter: &mut L2CFighterCommon) {
    acmd!({
        if(is_excute){
...
#[acmd_func(
    battle_object_category = BATTLE_OBJECT_CATEGORY_FIGHTER, 
    battle_object_kind = FIGHTER_KIND_SNAKE, 
    animation = "special_air_lw_blast",
    animcmd = "game_specialairlwblast")]
pub fn snake_c4_blast_air(fighter: &mut L2CFighterCommon) {
    acmd!({
        frame(Frame=2)
        if(is_excute){
            ArticleModule::generate_article(FIGHTER_SNAKE_GENERATE_ARTICLE_C4_SWITCH, false, 0)
...
pub fn snake_upb_start(fighter: &mut L2CFighterCommon) {
    acmd!({
        if(is_excute){
            CORRECT(GROUND_CORRECT_KIND_GROUND_CLIFF_STOP)
        }
